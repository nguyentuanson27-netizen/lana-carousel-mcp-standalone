...
function videoPanelHtml(){
  const cfg=project.videoSettings||{},enabled=project.videoEnabled;
  ...
  <p class="muted">T\u1eaft video kh\u00f4ng \u1ea3nh h\u01b0\u1edfng vi\u1ec7c t\u1ea3i b\u1ed9 \u1ea3nh.</p>
  <div class="fields">
    ...
    <label class="field">Hi\u1ec7u \u1ee9ng ch\u1eef<select id="videoTextAnimation"><option value="none">Gi\u1eef nguy\u00ean</option><option value="block" ${cfg.textAnimation==="block"?"selected":""}>C\u1ea3 kh\u1ed1i</option><option value="by-line" ${cfg.textAnimation==="by-line"?"selected":""}>Theo d\u00f2ng</option><option value="by-word" ${cfg.textAnimation==="by-word"?"selected":""}>Theo t\u1eeb</option><option value="typewriter" ${cfg.textAnimation==="typewriter"?"selected":""}>Typewriter</option></select></label>
    <label class="field wide">URL nh\u1ea1c n\u1ec1n HTTPS<input id="videoAudio" type="url" value="${esc(cfg.audioUrl||"")}" placeholder="https://...mp3"></label>
    <label class="field">\u00c2m l\u01b0\u1ee3ng <span class="range-value">${Math.round((cfg.audioVolume??.6)*100)}%</span><input id="videoVolume" type="range" min="0" max="100" value="${(cfg.audioVolume??.6)*100}"></label>
    <label class="toggle"><input id="videoSubtitles" type="checkbox" ${cfg.subtitles?"checked":""}> Ph\u1ee5 \u0111\u1ec1</label>
    <label class="toggle"><input id="videoBeat" type="checkbox" ${cfg.beatSync?"checked":""}> Beat sync</label>
    <label class="field">BPM<input id="videoBpm" type="number" min="40" max="240" value="${cfg.bpm||120}"></label>
    <label class="toggle"><input id="videoTts" type="checkbox" ${cfg.ttsEnabled?"checked":""}> Google TTS ti\u1ebfng Vi\u1ec7t</label>
  </div><button id="saveVideo" class="action">L\u01b0u c\u1ea5u h\u00ecnh video</button></div>
  <div class="card"><h2>C\u1ea3nh video</h2><div class="video-scenes">${project.slides.map((slide,index)=>{const v=slide.video||{};return `<div class="video-scene" data-video-slide="${slide.id}"><label class="toggle"><input data-v="enabled" type="checkbox" ${v.enabled!==false?"checked":""}> Slide ${slide.position}</label><strong>${esc(slide.headline)}</strong><label class="field">Th\u1ee9 t\u1ef1<input data-v="order" type="number" value="${v.order??index}"></label><label class="field">Gi\u00e2y<input data-v="duration" type="number" min=".5" max="15" step=".5" value="${v.duration||cfg.defaultSceneDuration||3}"></label><label class="field">Chuy\u1ec3n \u0111\u1ed9ng<select data-v="motion"><option value="zoom-in">Zoom in</option><option value="zoom-out" ${v.motion==="zoom-out"?"selected":""}>Zoom out</option><option value="pan-left" ${v.motion==="pan-left"?"selected":""}>Pan tr\u00e1i</option><option value="pan-right" ${v.motion==="pan-right"?"selected":""}>Pan ph\u1ea3i</option><option value="ken-burns" ${v.motion==="ken-burns"?"selected":""}>Ken Burns</option><option value="none" ${v.motion==="none"?"selected":""}>\u0110\u1ee9ng y\u00ean</option></select></label><button class="tool save-video-scene">L\u01b0u c\u1ea3nh</button></div>`}).join("")}</div></div>
...
$("video").onclick=async event=>{try{if(event.target.id==="saveVideo"){project=await json(`/api/projects/${projectId}/video`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:$("videoEnabled").checked,settings:{aspectRatio:$("videoAspect").value,fps:30,defaultSceneDuration:3,transition:$("videoTransition").value,motion:"zoom-in",textAnimation:$("videoTextAnimation").value,audioUrl:$("videoAudio").value,audioVolume:Number($("videoVolume").value)/100,subtitles:$("videoSubtitles").checked,beatSync:$("videoBeat").checked,bpm:Number($("videoBpm").value),ttsEnabled:$("videoTts").checked,ttsVoice:"vi-VN-Neural2-D",preset:$("videoPreset").value}})});render();return;}const row=event.target.closest("[data-video-slide]");if(event.target.closest(".save-video-scene")&&row){const get=n=>row.querySelector(`[data-v="${n}"]`);project=await json(`/api/projects/${projectId}/slides/${row.dataset.videoSlide}/video`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:get("enabled").checked,order:Number(get("order").value),duration:Number(get("duration").value),motion:get("motion").value})});render();return;}if(event.target.id==="startVideoRender"){const job=await json(`/api/projects/${projectId}/video-render-jobs`,{method:"POST"});pollVideoJob(job.id);}}catch(e){alert(e.message)}};
