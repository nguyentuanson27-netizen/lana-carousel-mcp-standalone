...
function videoPanelHtml(){
  const cfg=project.videoSettings||{},enabled=project.videoEnabled,voices=["Kore","Puck","Charon","Fenrir","Aoede","Leda","Orus","Zephyr","Achernar","Gacrux","Sulafat","Umbriel"];
  const voiceOptions=value=>voices.map(v=>`<option value="${v}" ${value===v?"selected":""}>${v}</option>`).join("");
  return `<div class="card"><div class="card-head"><div><div class="number">REMOTION · T\u00d9Y CH\u1eccN</div><h2>T\u1ea1o video t\u1eeb c\u00e1c slide \u0111\u00e3 duy\u1ec7t</h2></div><label class="toggle"><input id="videoEnabled" type="checkbox" ${enabled?"checked":""}> B\u1eadt video</label></div>
  <p class="muted">C\u1ea5u h\u00ecnh video s\u1ebd t\u1ef1 l\u01b0u khi chuy\u1ec3n b\u01b0\u1edbc.</p>
  <div class="fields">
    <label class="field">T\u1ef7 l\u1ec7<select id="videoAspect"><option value="vertical">9:16 TikTok/Reels</option><option value="square" ${cfg.aspectRatio==="square"?"selected":""}>1:1</option><option value="landscape" ${cfg.aspectRatio==="landscape"?"selected":""}>16:9</option></select></label>
    <label class="field">Preset<select id="videoPreset"><option value="fashion">Fashion</option><option value="tiktok" ${cfg.preset==="tiktok"?"selected":""}>TikTok nhanh</option><option value="minimal" ${cfg.preset==="minimal"?"selected":""}>T\u1ed1i gi\u1ea3n</option><option value="editorial" ${cfg.preset==="editorial"?"selected":""}>Editorial</option></select></label>
    <label class="field">Chuy\u1ec3n c\u1ea3nh<select id="videoTransition"><option value="fade">Fade</option><option value="cut" ${cfg.transition==="cut"?"selected":""}>Cut</option><option value="slide" ${cfg.transition==="slide"?"selected":""}>Slide</option><option value="zoom" ${cfg.transition==="zoom"?"selected":""}>Zoom</option></select></label>
    <label class="field">Hi\u1ec7u \u1ee9ng ch\u1eef<select id="videoTextAnimation"><option value="none">Gi\u1eef nguy\u00ean</option><option value="block" ${cfg.textAnimation==="block"?"selected":""}>Theo kh\u1ed1i</option><option value="by-line" ${cfg.textAnimation==="by-line"?"selected":""}>Theo d\u00f2ng</option><option value="by-word" ${cfg.textAnimation==="by-word"?"selected":""}>Theo t\u1eeb</option><option value="typewriter" ${cfg.textAnimation==="typewriter"?"selected":""}>G\u00f5 ch\u1eef</option></select></label>
    <label class="field wide">URL nh\u1ea1c n\u1ec1n HTTPS<input id="videoAudio" value="${esc(cfg.audioUrl||"")}" placeholder="https://...mp3"></label>
    <label class="field">\u00c2m l\u01b0\u1ee3ng nh\u1ea1c <strong id="videoVolumeValue">${Math.round((cfg.audioVolume??.6)*100)}%</strong><input id="videoVolume" type="range" min="0" max="100" value="${Math.round((cfg.audioVolume??.6)*100)}"></label>
    <label class="toggle"><input id="videoSubtitles" type="checkbox" ${cfg.subtitles?"checked":""}> Ph\u1ee5 \u0111\u1ec1</label><label class="toggle"><input id="videoBeat" type="checkbox" ${cfg.beatSync?"checked":""}> Beat sync</label>
    <label class="field">BPM<input id="videoBpm" type="number" min="40" max="240" value="${cfg.bpm||120}"></label>
    <label class="toggle"><input id="videoTts" type="checkbox" ${cfg.ttsEnabled?"checked":""}> B\u1eadt gi\u1ecdng \u0111\u1ecdc TTS</label>
    <label class="field">Nh\u00e0 cung c\u1ea5p<select id="videoTtsProvider"><option value="google">Google TTS</option><option value="gemini" ${cfg.ttsProvider==="gemini"?"selected":""}>Gemini TTS</option></select></label>
    <label class="field">T\u1ed1c \u0111\u1ed9 \u0111\u1ecdc <strong id="videoTtsSpeedValue">${Number(cfg.ttsSpeed||1).toFixed(2)}x</strong><input id="videoTtsSpeed" type="range" min="0.5" max="2" step="0.05" value="${cfg.ttsSpeed||1}"></label>
    <label class="field">\u00c2m l\u01b0\u1ee3ng gi\u1ecdng <strong id="videoTtsVolumeValue">${Math.round((cfg.ttsVolume??1)*100)}%</strong><input id="videoTtsVolume" type="range" min="0" max="100" value="${Math.round((cfg.ttsVolume??1)*100)}"></label>
    <label class="field">Model Gemini<select id="geminiModel"><option value="gemini-2.5-flash-preview-tts">Gemini 2.5 Flash TTS</option><option value="gemini-2.5-pro-preview-tts" ${cfg.geminiModel==="gemini-2.5-pro-preview-tts"?"selected":""}>Gemini 2.5 Pro TTS</option><option value="gemini-3.1-flash-tts-preview" ${cfg.geminiModel==="gemini-3.1-flash-tts-preview"?"selected":""}>Gemini 3.1 Flash TTS</option></select></label>
    <label class="toggle"><input id="geminiMultiSpeaker" type="checkbox" ${cfg.geminiMultiSpeaker?"checked":""}> Nhi\u1ec1u gi\u1ecdng (t\u1ed1i \u0111a 2)</label>
    <label class="field">Nh\u00e2n v\u1eadt 1<input id="geminiSpeaker1Name" value="${esc(cfg.geminiSpeaker1Name||"Người dẫn")}"></label><label class="field">Gi\u1ecdng 1<select id="geminiSpeaker1Voice">${voiceOptions(cfg.geminiSpeaker1Voice||"Kore")}</select></label>
    <label class="field">Nh\u00e2n v\u1eadt 2<input id="geminiSpeaker2Name" value="${esc(cfg.geminiSpeaker2Name||"Khách mời")}"></label><label class="field">Gi\u1ecdng 2<select id="geminiSpeaker2Voice">${voiceOptions(cfg.geminiSpeaker2Voice||"Puck")}</select></label>
    <label class="field wide">Phong c\u00e1ch gi\u1ecdng Gemini<textarea id="geminiStylePrompt" placeholder="V\u00ed d\u1ee5: gi\u1ecdng th\u00e2n thi\u1ec7n, n\u0103ng \u0111\u1ed9ng, ng\u1eaft ngh\u1ec9 t\u1ef1 nhi\u00ean">${esc(cfg.geminiStylePrompt||"")}</textarea></label>
  </div><button id="saveVideo" class="action">L\u01b0u c\u1ea5u h\u00ecnh video</button></div>
  <div class="card"><h2>C\u1ea3nh video</h2><div class="video-scenes">${project.slides.map((slide,index)=>{const v=slide.video||{};return `<div class="video-scene" data-video-slide="${slide.id}"><label class="toggle"><input data-v="enabled" type="checkbox" ${v.enabled!==false?"checked":""}> Slide ${slide.position}</label><strong>${esc(slide.headline)}</strong><label class="field">Th\u1ee9 t\u1ef1<input data-v="order" type="number" value="${v.order??index}"></label><label class="field">Gi\u00e2y<input data-v="duration" type="number" min=".5" max="15" step=".5" value="${v.duration||cfg.defaultSceneDuration||3}"></label><label class="field">Chuy\u1ec3n \u0111\u1ed9ng<select data-v="motion"><option value="zoom-in">Zoom in</option><option value="zoom-out" ${v.motion==="zoom-out"?"selected":""}>Zoom out</option><option value="pan-left" ${v.motion==="pan-left"?"selected":""}>Pan tr\u00e1i</option><option value="pan-right" ${v.motion==="pan-right"?"selected":""}>Pan ph\u1ea3i</option><option value="ken-burns" ${v.motion==="ken-burns"?"selected":""}>Ken Burns</option><option value="none" ${v.motion==="none"?"selected":""}>\u0110\u1ee9ng y\u00ean</option></select></label><label class="field">Nh\u00e2n v\u1eadt<select data-v="speaker"><option value="speaker1">Gi\u1ecdng 1</option><option value="speaker2" ${v.speaker==="speaker2"?"selected":""}>Gi\u1ecdng 2</option></select></label><button class="tool save-video-scene">L\u01b0u c\u1ea3nh</button></div>`}).join("")}</div></div>
  <div class="card"><h2>Render MP4</h2><video id="videoPreviewPlayer" class="hidden" controls playsinline style="width:min(420px,100%);aspect-ratio:9/16;background:#111;border-radius:14px"></video><div class="video-actions"><button id="startVideoRender" class="action" ${enabled?"":"disabled"}>Render video</button><span id="videoJobText" class="muted"></span><a id="videoDownload" class="action hidden">T\u1ea3i MP4</a></div><div class="render-progress"><span id="videoJobProgress" style="width:0%"></span></div></div>`;
}
...
function videoSettingsFromDom(){return {aspectRatio:$("videoAspect").value,fps:30,defaultSceneDuration:3,transition:$("videoTransition").value,motion:"zoom-in",textAnimation:$("videoTextAnimation").value,audioUrl:$("videoAudio").value,audioVolume:Number($("videoVolume").value)/100,subtitles:$("videoSubtitles").checked,beatSync:$("videoBeat").checked,bpm:Number($("videoBpm").value),ttsEnabled:$("videoTts").checked,ttsProvider:$("videoTtsProvider").value,ttsVoice:"vi-VN-Neural2-D",ttsSpeed:Number($("videoTtsSpeed").value),ttsVolume:Number($("videoTtsVolume").value)/100,geminiModel:$("geminiModel").value,geminiMultiSpeaker:$("geminiMultiSpeaker").checked,geminiSpeaker1Name:$("geminiSpeaker1Name").value||"Nguoi dan",geminiSpeaker1Voice:$("geminiSpeaker1Voice").value,geminiSpeaker2Name:$("geminiSpeaker2Name").value||"Khach moi",geminiSpeaker2Voice:$("geminiSpeaker2Voice").value,geminiStylePrompt:$("geminiStylePrompt").value,preset:$("videoPreset").value};}
function sceneSettingsFromRow(row){const get=n=>row.querySelector(`[data-v="${n}"]`);return {enabled:get("enabled").checked,order:Number(get("order").value),duration:Number(get("duration").value),motion:get("motion").value,speaker:get("speaker").value};}
...
$("video").oninput=event=>{if(event.target.id==="videoVolume")$("videoVolumeValue").textContent=event.target.value+"%";if(event.target.id==="videoTtsSpeed")$("videoTtsSpeedValue").textContent=Number(event.target.value).toFixed(2)+"x";if(event.target.id==="videoTtsVolume")$("videoTtsVolumeValue").textContent=event.target.value+"%";};
$("video").onclick=async event=>{try{if(event.target.id==="saveVideo"){await saveAllVideoFromDom();render();return;}const row=event.target.closest("[data-video-slide]");if(event.target.closest(".save-video-scene")&&row){await saveVideoSceneRow(row);render();return;}if(event.target.id==="startVideoRender"){await saveAllVideoFromDom();render();const job=await json(`/api/projects/${projectId}/video-render-jobs`,{method:"POST"});pollVideoJob(job.id);}}catch(e){alert(e.message)}};
async function pollVideoJob(id){try{const job=await json(`/api/video-render-jobs/${id}`);$("videoJobText").textContent=`${job.status} · ${job.progress}%`;$("videoJobProgress").style.width=`${job.progress}%`;if(job.status==="READY"){$("videoPreviewPlayer").src=job.downloadUrl;$("videoPreviewPlayer").classList.remove("hidden");$("videoDownload").href=job.downloadUrl;$("videoDownload").classList.remove("hidden");return}if(job.status==="FAILED")throw Error(job.error);setTimeout(()=>pollVideoJob(id),1500)}catch(e){alert(e.message)}}
