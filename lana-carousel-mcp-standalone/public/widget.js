...
function videoPanelHtml(){const cfg=project.videoSettings||{},enabled=project.videoEnabled;return `<div class="card"><div class="card-head"><div><div class="number">Remotion · tÃ¹y chá»n</div><h2>Táº¡o video tá»« cÃ¡c slide Ä‘Ã£ duyá»‡t</h2></div><label class="toggle"><input id="videoEnabled" type="checkbox" ${enabled?"checked":""}> Báº­t video</label></div><p class="muted">Táº¯t video khÃ´ng áº£nh hÆ°á»Ÿng táº£i bá»™ áº£nh.</p><div class="fields"><label class="field">Tá»· lá»‡<select id="videoAspect"><option value="vertical">9:16 TikTok/Reels</option><option value="square" ${cfg.aspectRatio==="square"?"selected":""}>1:1</option><option value="landscape" ${cfg.aspectRatio==="landscape"?"selected":""}>16:9</option></select></label><label class="field">Preset<select id="videoPreset"><option value="fashion">Fashion</option><option value="tiktok" ${cfg.preset==="tiktok"?"selected":""}>TikTok nhanh</option><option value="minimal" ${cfg.preset==="minimal"?"selected":""}>Tá»‘i giáº£n</option><option value="editorial" ${cfg.preset==="editorial"?"selected":""}>Editorial</option></select></label><label class="field">Chuyá»ƒn cáº£nh<select id="videoTransition"><option value="fade">Fade</option><option value="cut" ${cfg.transition==="cut"?"selected":""}>Cut</option><option value="slide" ${cfg.transition==="slide"?"selected":""}>Slide</option><option value="zoom" ${cfg.transition==="zoom"?"selected":""}>Zoom</option></select></label><label class="field">Hiá»‡u á»©ng chá»¯<select id="videoTextAnimation"><option value="none">Giá»¯ nguyÃªn</option><option value="block" ${cfg.textAnimation==="block"?"selected":""}>Cáº£ khá»‘i</option><option value="by-line" ${cfg.textAnimation==="by-line"?"selected":""}>Theo dÃ²ng</option><option value="by-word" ${cfg.textAnimation==="by-word"?"selected":""}>Theo tá»«</option><option value="typewriter" ${cfg.textAnimation==="typewriter"?"selected":""}>Typewriter</option></select></label><label class="field wide">URL nháº¡c ná»n HTTPS<input id="videoAudio" type="url" value="${esc(cfg.audioUrl||"")}" placeholder="https://...mp3"></label><label class="field">Ã‚m lÆ°á»£ng <span class="range-value">${Math.round((cfg.audioVolume??.6)*100)}%</span><input id="videoVolume" type="range" min="0" max="100" value="${(cfg.audioVolume??.6)*100}"></label><label class="toggle"><input id="videoSubtitles" type="checkbox" ${cfg.subtitles?"checked":""}> Phá»¥ Ä‘á»</label><label class="toggle"><input id="videoBeat" type="checkbox" ${cfg.beatSync?"checked":""}> Beat sync</label><label class="field">BPM<input id="videoBpm" type="number" min="40" max="240" value="${cfg.bpm||120}"></label><label class="toggle"><input id="videoTts" type="checkbox" ${cfg.ttsEnabled?"checked":""}> Google TTS (náº¿u VPS cÃ³ credentials)</label></div><button id="saveVideo" class="action">LÆ°u cáº¥u hÃ¬nh video</button></div><div class="card"><h2>Cáº£nh video</h2><div class="video-scenes">${project.slides.map((slide,index)=>{const v=slide.video||{};return `<div class="video-scene" data-video-slide="${slide.id}"><label class="toggle"><input data-v="enabled" type="checkbox" ${v.enabled!==false?"checked":""}> Slide ${slide.position}</label><strong>${esc(slide.headline)}</strong><label class="field">Thá»© tá»±<input data-v="order" type="number" value="${v.order??index}"></label><label class="field">GiÃ¢y<input data-v="duration" type="number" min=".5" max="15" step=".5" value="${v.duration||cfg.defaultSceneDuration||3}"></label><label class="field">Chuyá»ƒn Ä‘á»™ng<select data-v="motion"><option value="zoom-in">Zoom in</option><option value="zoom-out" ${v.motion==="zoom-out"?"selected":""}>Zoom out</option><option value="pan-left" ${v.motion==="pan-left"?"selected":""}>Pan trÃ¡i</option><option value="pan-right" ${v.motion==="pan-right"?"selected":""}>Pan pháº£i</option><option value="ken-burns" ${v.motion==="ken-burns"?"selected":""}>Ken Burns</option><option value="none" ${v.motion==="none"?"selected":""}>Äá»©ng yÃªn</option></select></label><button class="tool save-video-scene">LÆ°u cáº£nh</button></div>`}).join("")}</div></div><div class="card"><h2>Render MP4</h2><video id="videoPreviewPlayer" class="hidden" controls playsinline style="width:min(420px,100%);aspect-ratio:9/16;background:#111;border-radius:14px"></video><div class="video-actions"><button id="startVideoRender" class="action" ${enabled?"":"disabled"}>Render video</button><span id="videoJobText" class="muted"></span><a id="videoDownload" class="action hidden">Táº£i MP4</a></div><div class="render-progress"><span id="videoJobProgress" style="width:0%"></span></div></div>`}
function render() {
