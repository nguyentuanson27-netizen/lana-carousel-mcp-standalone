...
const Caption=({segment,settings})=>{
 ...
 return <div style={{position:"absolute",left:`${settings.subtitleX??50}%`,top:`${settings.subtitlePosition??86}%`,transform:"translate(-50%,-50%)",width:wordMode?"auto":"88%",minWidth:wordMode?160:undefined,maxWidth:"94%",textAlign:"center",fontFamily:FONT_STACKS[settings.subtitleFont]||settings.subtitleFont||FONT_STACKS["TikTok Sans"],fontSize:settings.subtitleSize||52,fontWeight:700,lineHeight:1.18,color:settings.subtitleColor||"#fff",background:`${settings.subtitleBackgroundColor||"#000"}${Math.round((settings.subtitleBackgroundOpacity??.72)*255).toString(16).padStart(2,"0")}`,padding:"14px 20px",borderRadius:18,whiteSpace:"pre-wrap",overflowWrap:"anywhere"}}>{wordMode?<span style={{display:"inline-block",transform:"scale(1.06)"}}>{wordParts[active]||""}</span>:words.map((word,index)=>{if(word.trim())seen++;return <span key={index} style={{color:settings.subtitleStyle==="karaoke"&&seen===active?"#ffdf70":undefined}}>{word}</span>})}</div>;
};
...
export const LanaAnalyzedVideoComposition=()=> <Composition id="LanaAnalyzedVideo" component={AnalyzedVideo} width={1080} height={1920} fps={30} durationInFrames={300} defaultProps={{sourceVideoUrl:"",sourceDuration:0,voiceDuration:0,segments:[],settings:{}}} calculateMetadata={({props})=>{const fps=30,d=Math.max(Number(props?.sourceDuration||0),Number(props?.voiceDuration||0),...(props?.segments||[]).map(segment=>Number(segment.end||0)),1);return{fps,durationInFrames:Math.ceil(d*fps)}}}/>;
