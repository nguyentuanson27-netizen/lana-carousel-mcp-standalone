...
const Caption=({text,style="karaoke",position=88})=>{
...
const Scene=({scene})=>{
 ...
 return <AbsoluteFill style={{opacity,overflow:"hidden",background:"#111",transform:transitionTransform}}>
  ...
  {(scene.textLayers||[]).filter(x=>x.enabled!==false).map((layer,i)=><RichLayer key={layer.id||i} layer={layer} animation={layer.animation||scene.textAnimation}/>)}
  {scene.subtitles?<Caption text={scene.caption||scene.body||scene.headline} style={scene.subtitleStyle} position={scene.subtitlePosition}/>:null}
 </AbsoluteFill>;
};
