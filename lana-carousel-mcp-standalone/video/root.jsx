...
const motionTransform = (motion,frame,duration) => {
...
const Caption=({text})=><div style={{position:"absolute",left:"7%",right:"7%",bottom:"4%",padding:"16px 22px",borderRadius:18,background:"#000b",color:"#fff",font:"700 38px TikTok Sans,Arial",textAlign:"center"}}>{text}</div>;
const Scene=({scene})=>{
 ...
 return <AbsoluteFill style={{opacity,overflow:"hidden",background:"#111",transform:transitionTransform}}>
  <Img src={scene.imageUrl} style={{width:"100%",height:"100%",objectFit:"cover",transform:motionTransform(scene.motion,frame,durationInFrames)}}/>
  {(scene.textLayers||[]).filter(x=>x.enabled!==false).map((layer,i)=><RichLayer key={layer.id||i} layer={layer} animation={scene.textAnimation}/>)}
