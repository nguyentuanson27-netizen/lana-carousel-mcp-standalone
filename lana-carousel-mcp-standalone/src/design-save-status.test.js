...
test("Carousel UI reads persisted design save state", async () => {
  ...
  assert.match(widget, /designDirty = new Set\(\)/u);
  assert.match(widget, /function markDesignDirty\(slideId\)/u);
  assert.match(widget, /slide\.designSaved && !designDirty\.has\(slide\.id\)/u);
  assert.match(widget, /designDirty\.delete\(slideId\)/u);
  assert.match(widget, /slides\/\$\{slideId\}\/design/u);
  assert.match(stitch, /card\.dataset\.designSaved === 'true' \? 'done' : 'pending'/u);
  assert.match(stitch, /attributeFilter: \['class', 'disabled', 'data-design-saved'\]/u);
