...
async function postToken(baseUrl, values) {
...
test("invalid requests carrying a registered client id cannot starve its valid token exchange", async t => {
  ...
  const consent = consumeConsentRequest(createConsentRequest(request, "user:targeted-limit"), "user:targeted-limit");
  const validCode = issueAuthorizationCode(consent);
