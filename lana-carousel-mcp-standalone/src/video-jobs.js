...
function propsFor(project){
 ...
 let scenes=project.slides.map((slide,index)=>{
  ...
  return{id:slide.id,enabled:s.enabled!==false,order:Number(s.order??index),duration,motion:(s.motion==="ken-burns"&&cfg.smartKenBurns)?"smart-ken-burns":s.motion||cfg.motion||preset.motion||"zoom-in",transition:s.transition||cfg.transition||preset.transition||"fade",textAnimation:s.textAnimation||cfg.textAnimation||preset.textAnimation||"none",subtitles:s.subtitles??cfg.subtitles??false,subtitleStyle:s.subtitleStyle||cfg.subtitleStyle||"karaoke",subtitlePosition:Number(s.subtitlePosition??cfg.subtitlePosition??88),subtitleFont:s.subtitleFont||cfg.subtitleFont||"TikTok Sans",subtitleColor:s.subtitleColor||cfg.subtitleColor||"#FFFFFF",subtitleBackgroundColor:s.subtitleBackgroundColor||cfg.subtitleBackgroundColor||"#000000",subtitleBackgroundOpacity:Number(s.subtitleBackgroundOpacity??cfg.subtitleBackgroundOpacity??.72),subtitleSize:Number(s.subtitleSize??cfg.subtitleSize??38),showSlideTitle:s.showSlideTitle??cfg.showSlideTitle??false,caption:s.caption??slide.body??"",headline:slide.headline,body:slide.body,textLayers:layers,imageUrl:asset?.publicUrl||"",focusX:Number(s.focusX??slide.cropX??50),focusY:Number(s.focusY??slide.cropY??50),kenBurnsIntensity:Number(s.kenBurnsIntensity??cfg.kenBurnsIntensity??.14)};
