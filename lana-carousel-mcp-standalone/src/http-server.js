...
app.patch("/api/projects/:projectId/video", (req,res)=>{try{const settings=z.object({aspectRatio:z.enum(["vertical","square","landscape"]).default("vertical"),fps:z.number().int().min(24).max(60).default(30),defaultSceneDuration:z.number().min(.5).max(15).default(3),transition:z.enum(["cut","fade","slide","zoom"]).default("fade"),motion:z.enum(["none","zoom-in","zoom-out","pan-left","pan-right","ken-burns"]).default("zoom-in"),textAnimation:z.enum(["none","block","by-line","by-word","typewriter"]).default("none"),audioUrl:z.union([z.string().url(),z.literal("")]).default(""),audioVolume:z.number().min(0).max(1).default(.6),subtitles:z.boolean().default(false),beatSync:z.boolean().default(false),bpm:z.number().min(40).max(240).default(120),ttsEnabled:z.boolean().default(false),ttsProvider:z.enum(["google","gemini"]).default("google"),ttsVoice:z.string().max(100).default("vi-VN-Neural2-D"),ttsSpeed:z.number().min(.5).max(2).default(1),ttsVolume:z.number().min(0).max(1).default(1),geminiModel:z.enum(["gemini-3.1-flash-tts-preview","gemini-2.5-flash-preview-tts","gemini-2.5-pro-preview-tts"]).default("gemini-2.5-flash-preview-tts"),geminiMultiSpeaker:z.boolean().default(false),geminiSpeaker1Name:z.string().min(1).max(50).default("Nguoi dan"),geminiSpeaker1Voice:z.string().min(1).max(50).default("Kore"),geminiSpeaker2Name:z.string().min(1).max(50).default("Khach moi"),geminiSpeaker2Voice:z.string().min(1).max(50).default("Puck"),geminiStylePrompt:z.string().max(500).default(""),preset:z.enum(["fashion","tiktok","minimal","editorial"]).default("fashion")}).parse(req.body.settings||{});res.json(updateProjectVideo({projectId:req.params.projectId,enabled:Boolean(req.body.enabled),settings}));}catch(error){const safe=publicError(error);res.status(safe.status).json(safe);}});
